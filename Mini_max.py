...
def calculate_depth(seconds_left):
    if seconds_left > 1:
        return int(seconds_left / 50)
    return 1


game_states = []
attackers_map = np.zeros(64, dtype=int)


# Precompute attackers for each square

def precompute_attackers(board, color):
    for square in chess.SQUARES:
        attackers_map[square] = len(board.attackers(color, square))
...
def lookup_transposition(board_key):
    return transposition_table.get(board_key)


def store_transposition(board_key, score):
...
class MyAgent(Player):
    ...
    def order_moves(self, move_actions, board):
        ...
        piece_to_value = {'-': 0, 'p': 1, 'n': 8, 'b': 9, 'r': 10, 'q': 11, 'k': 12,
                          'P': 1, 'N': 8, 'B': 9, 'R': 10, 'Q': 11, 'K': 12}
    ...
    def minimax(self, board, depth, is_maximizing, alpha, beta):
        ...
        if depth == 0 or board.is_game_over():
            return self.evaluate_board(board, is_maximizing)
        if is_maximizing:
            max_eval = -math.inf
            ordered_moves = self.order_moves(board.legal_moves, board)
            count = 0
            for move in ordered_moves:
                ...
                if beta <= alpha:
                    break
                if count == 25: #use this to limit the number of moves searched
                     break
            return max_eval
        else:
            min_eval = math.inf
            ...
            for move in ordered_moves:
                ...
                if beta <= alpha:
                    break
                if count1 == 25: #use this to limit the number of moves searched
                    break
            return min_eval
    ...
    def choose_move(self, move_actions: List[chess.Move], seconds_left: float) -> Optional[chess.Move]:
        ...
        max_depth = calculate_depth(seconds_left)  # Calculate the maximum depth based on the remaining time
        best_move = None
        best_score = -math.inf
        alpha = -math.inf
        beta = math.inf
        ...
        for move in ordered_moves:
            ...
            score = self.minimax(self.board, 2, True, alpha, beta)
            if score > best_score:
                best_score = score
                best_move = move
